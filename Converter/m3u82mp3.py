...
def run_from_cmd(input_filepath, output_filepath):
    """Run converter from command line."""

    if input_filepath:
        input = open('../Converter/m3u8Files/' + input_filepath + '.m3u8', 'r')
    else:
        input = sys.stdin

    if output_filepath:
        output = open('Downloaded_Music/'+output_filepath + '.mp3', 'wb')
    else:
